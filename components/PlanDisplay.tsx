...
interface PlanDisplayProps {
  ...
  onFollowPlan?: () => void; // Optional: Only for workout plans
}

const PlanDisplay: React.FC<PlanDisplayProps> = ({ content, onReset, title, onRegenerate, currentPlanType, onCrossNavigate, onFollowPlan }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl border border-gray-100 print-content">
      ...
      {/* FOLLOW PLAN BUTTON (WORKOUT ONLY) */}
      {currentPlanType === 'workout' && onFollowPlan && (
        <div className="bg-gray-900 p-6 flex flex-col items-center gap-3 no-print border-t border-gray-800">
           <p className="text-gray-400 font-bold text-sm uppercase tracking-wide">
             Ready to train?
           </p>
           <button 
             onClick={onFollowPlan}
             className="w-full sm:w-auto bg-red-600 text-white border-2 border-red-600 px-8 py-3 rounded-full font-extrabold text-lg hover:bg-red-700 transition-all transform hover:-translate-y-1 shadow-lg flex items-center justify-center gap-2"
           >
             🚀 Follow this Plan in Tracker
           </button>
        </div>
      )}
